...
def _fmt_bias_section(bias_report: dict[str, Any]) -> str:
    ...
    lines = [
        f"Overall Psychological Health Score: {bias_report.get('overallHealthScore', '?')}/100",
        "",
    ]
    ...
    for s in bias_report.get("symbolHealthScores", [])[:10]:
        counts_str = " | ".join(
            f"{k.replace('_', ' ')[:4]}: {v}" for k, v in s.get("biasCounts", {}).items() if v > 0
        )
        lines.append(
...
def build_prompt(payload: dict[str, Any]) -> str:
    ...
    bias_section_instruction = (
        ...
        if has_biases
        else "No biases were algorithmically detected in this period. Note this positively and comment "
