...
function ChangeBadge({ value }: { value: number }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-0.5 font-semibold tabular-nums ${positive ? 'text-emerald-400' : 'text-red-400'}`}
    >
...
function MoverCard({ ticker, rank }: { ticker: LiveTicker; rank: number }) {
  ...
  return (
    <div
      className={`flex-1 min-w-[140px] rounded-xl border px-3.5 py-2.5 ${
        positive
    ...
    >
      ...
      <div className="text-xs font-semibold text-white/90 truncate">
        {ticker.underlying_asset_symbol ?? ticker.symbol}
      </div>
      <div className="text-[11px] text-white/40 mt-0.5 tabular-nums">
        {fmtPrice(ticker.mark_price)}
      </div>
    </div>
  )
}
...
export default function LiveMarketsPage() {
  ...
  const colHeader = (label: string, key: SortKey, align: 'left' | 'right' = 'right') => (
    ...
    >
      <span
        className={`inline-flex items-center gap-1 text-xs uppercase tracking-widest text-white/40 group-hover:text-white/70 transition-colors ${align === 'right' ? 'flex-row-reverse' : ''}`}
      >
        ...
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-white/35 mt-1">
              Delta Exchange India — realtime websocket data
            </p>
          </div>
          ...
          {!loading && rows.length > 0 && (
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <div className="text-center px-3 py-1.5 rounded-xl bg-emerald-500/[0.08] border border-emerald-500/20">
                ...
                <div className="text-[10px] text-emerald-400/60 uppercase tracking-wider">
                  Gainers
                </div>
              </div>
            ...
            {topGainers.length > 0 && (
              <div>
                ...
                <div className="flex gap-2 flex-wrap">
                  {topGainers.map((t, i) => (
                    <MoverCard key={t.symbol} ticker={t} rank={i + 1} />
                  ))}
                </div>
              </div>
            )}
              ...
              <table className="min-w-full text-sm">
                <thead className="bg-white/[0.025] border-b border-white/[0.06]">
                  <tr>
                    ...
                    <th className="text-left px-3 py-3 text-xs uppercase tracking-widest text-white/40">
                      Type
                    </th>
                    <th className="text-left px-3 py-3 text-xs uppercase tracking-widest text-white/40">
                      Asset
