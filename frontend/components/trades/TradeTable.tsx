  ...
  if (!trades.length) {
    return (
      <div className="bg-[#0f1117] border border-white/[0.07] rounded-xl p-12 text-center">
        <div className="text-white/20 text-sm">No fills yet. Sync from Delta Exchange or add a trade manually.</div>
      </div>
    )
  }
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="text-white/30 text-xs border-b border-white/[0.05]">
              {['Symbol', 'Side', 'Fill Price', 'Qty', 'Realized P&L', 'Details', 'Date'].map(h => (
            ...
            {trades.map((t) => (
              <tr key={t.id} className="border-b border-white/[0.04] hover:bg-white/[0.02] transition-colors">
                ...
                <td className="px-4 py-3">
                  <span className={`text-xs px-2 py-0.5 rounded font-medium ${t.direction === 'long' ? 'text-emerald-400 bg-emerald-500/10' : 'text-red-400 bg-red-500/10'}`}>
                ...
                <td className={`px-4 py-3 font-semibold tabular-nums ${t.pnl > 0 ? 'text-emerald-400' : t.pnl < 0 ? 'text-red-400' : 'text-white/30'}`}>
                  {t.pnl === 0
