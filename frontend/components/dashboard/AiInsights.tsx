...
import {
  BiasSummary, BiasInstance, SymbolBiasReport, GlobalBiasReport,
} from '@/lib/bias-analysis'
import { BIAS_COLORS as COLORS, SEVERITY_LABEL } from '@/lib/bias-colors'
import { Sparkles, Loader2, AlertCircle, ChevronRight, Brain, Activity, RefreshCw, TrendingUp, DollarSign } from 'lucide-react'
...
function InstanceList({ instances, type }: { instances: BiasInstance[]; type: string }) {
  ...
  if (instances.length === 0) return (
    <p className="text-xs text-white/25 italic pl-1">No instances detected</p>
  )
  return (
      ...
      {shown.map((inst, i) => (
        <div key={i} className={`flex gap-2 text-xs px-2 py-1.5 rounded-lg ${c.bg}`}>
          ...
          <div className="min-w-0">
            ...
            <span className="text-white/25 ml-1.5 text-[10px]">{inst.entryTime.slice(0, 16).replace('T', ' ')}</span>
          </div>
        </div>
      ))}
      {instances.length > 3 && (
    ...
    <div className="space-y-5">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <div className="text-xs text-white/40 mt-0.5">{report.totalTrades} fills · {totalInstances} bias signals detected</div>
        </div>
      </div>
  ...
  return (
    <div className="space-y-4 text-sm leading-relaxed text-white/80">
      {sections.map((section, i) => {
        ...
        if (!heading) return <p key={i} className="text-white/70">{body}</p>
        return (
          <div key={i}>
...
function PnlCalendar({ trades }: { trades: Trade[] }) {
  ...
  const maxAbs = useMemo(() => {
    let m = 0
    byDate.forEach(v => { if (Math.abs(v.pnl) > m) m = Math.abs(v.pnl) })
    return Math.max(m, 1)
  }, [byDate])
  ...
  return (
    <div className="bg-[#0f1117] border border-white/[0.07] rounded-xl p-4">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-xs text-white/30 uppercase tracking-widest font-medium">30-Day P&L Calendar</span>
      </div>
      <div className="flex flex-wrap gap-1.5">
        {days.map(day => {
          ...
          return (
            ...
            >
              <span className="text-[9px] text-white/50 font-medium select-none">{parseInt(day.slice(-2))}</span>
            </div>
          )
        })}
      </div>
...
function CommissionPanel({ stats, trades }: { stats: ExtendedDashboardStats; trades: Trade[] }) {
  const grossWin  = useMemo(() => trades.filter(t => t.pnl > 0).reduce((s, t) => s + t.pnl, 0), [trades])
  ...
  return (
    <div className="bg-[#0f1117] border border-white/[0.07] rounded-xl p-4">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-xs text-white/30 uppercase tracking-widest font-medium">Commission Impact</span>
      </div>
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: 'Gross P&L',   value: `${stats.totalPnl >= 0 ? '+' : ''}${stats.totalPnl.toFixed(2)}`,  cls: stats.totalPnl >= 0 ? 'text-emerald-400' : 'text-red-400' },
          { label: 'Commission',  value: `-${commission.toFixed(2)}`,                                         cls: 'text-amber-400' },
          { label: '% of Gross',  value: `${commissionPct.toFixed(1)}%`,                                     cls: commissionPct > 15 ? 'text-red-400' : commissionPct > 8 ? 'text-amber-400' : 'text-white/70' },
      ...
      {commissionPct > 15 && (
        <p className="text-xs text-red-400/70 mt-3">
          ⚠ Commission is {commissionPct.toFixed(0)}% of your gross wins — high frequency is eating significantly into returns. Consider reducing trade count.
        </p>
      )}
    </div>
  )
...
function BestSetups({ trades }: { trades: Trade[] }) {
  const setups = useMemo(() => {
    ...
    trades.filter(t => t.pnl !== 0).forEach(t => {
      ...
      map.set(key, { wins: cur.wins + (t.pnl > 0 ? 1 : 0), total: cur.total + 1, pnl: cur.pnl + t.pnl })
    })
    return [...map.entries()]
      .filter(([, v]) => v.total >= 5)
  ...
  return (
    <div className="bg-[#0f1117] border border-white/[0.07] rounded-xl p-4">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-xs text-white/30 uppercase tracking-widest font-medium">Your Best Setups (≥5 samples)</span>
      </div>
        ...
        {setups.map((s, i) => {
          ...
          return (
            <div key={s.key} className="flex items-center gap-3">
              ...
              <span className={`text-[10px] px-1.5 py-0.5 rounded font-bold shrink-0 ${isLong ? 'bg-emerald-500/10 text-emerald-400' : 'bg-red-500/10 text-red-400'}`}>
                {isLong ? 'LONG' : 'SHORT'}
              </span>
              ...
              <span className="text-xs font-semibold text-emerald-400 w-10 text-right tabular-nums">{s.winRate.toFixed(0)}%</span>
              <span className="text-[10px] text-white/25 w-12 text-right tabular-nums">{s.total}t</span>
            </div>
          )
        })}
      </div>
...
export function AiInsights({ stats, symbolStats, hourStats, trades, biasReport }: Props) {
  ...
  const activeSymbol: SymbolBiasReport | undefined =
    selectedSymbol
        ...
        body: JSON.stringify({
          ...
          biasReport: {
            ...
            biases: report.biases.map(b => ({
              type: b.type, label: b.label, count: b.instances.length,
              score: b.score, severity: b.severity,
              topInstances: b.instances.slice(0, 5).map(i => ({
                symbol: i.symbol, reason: i.reason, severity: i.severity,
              })),
            })),
            symbolHealthScores: report.symbolReports.map(r => ({
              symbol: r.symbol, healthScore: r.healthScore,
    ...
    <div className="space-y-4">
        ...
        <div className="flex items-start gap-5">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-xs font-semibold text-white/60 uppercase tracking-widest">Psychological Profile — All Coins</span>
            </div>
        ...
        <div className="bg-[#0f1117] border border-white/[0.07] rounded-xl overflow-hidden">
          <div className="flex items-center gap-1 px-3 pt-3 pb-0 flex-wrap">
            ...
            {report.symbolReports.map(r => {
              ...
              const dotColor = worstBias >= 60 ? 'bg-red-500' : worstBias >= 30 ? 'bg-yellow-500' : 'bg-green-500'
              return (
      ...
      <div className="bg-[#0f1117] border border-white/[0.07] rounded-xl p-4">
        <div className="flex items-center justify-between gap-4">
          <div>
            <div className="text-xs text-white/40 uppercase tracking-widest font-medium mb-1">AI Deep Report</div>
          ...
          <button onClick={generate} disabled={loading || !hasTrades}
      ...
      {analysis && (
        <div className="bg-[#0f1117] border border-white/[0.07] rounded-xl p-5">
          <div className="flex items-center gap-2 mb-4 pb-3 border-b border-white/[0.06]">
            ...
            <span className="text-xs text-violet-400/80 font-semibold uppercase tracking-widest">Gemini Analysis</span>
