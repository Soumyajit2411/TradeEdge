  ...
  return (
    <div className={`${cfg.bg} border ${cfg.border} rounded-xl p-4 relative`}>
      ...
      <div className="flex items-start gap-3">
        ...
        <button
          onClick={onDismiss}
          className="text-white/15 hover:text-white/40 transition-colors text-lg leading-none shrink-0"
        >
          ×
        </button>
      </div>
...
function DailyGuard({ trades }: { trades: Trade[] }) {
  ...
  const todayPnl = useMemo(
    () => trades.filter((t) => t.entry_time.startsWith(today)).reduce((s, t) => s + t.pnl, 0),
    [trades, today]
  )
  ...
  return (
    <div className={`${status.bg} border ${status.border} rounded-xl p-4`}>
      <div className="flex items-center justify-between mb-3">
        ...
        <span
          className={`text-xs font-bold px-2 py-0.5 rounded-full ${status.bg} ${status.cls} border ${status.border}`}
        >
          {status.label}
        </span>
      </div>
      <div className="grid grid-cols-3 gap-3 mb-3">
        <div className="text-center">
          ...
          <div className={`text-xl font-bold ${tradeRatio >= 1.5 ? status.cls : 'text-white'}`}>
            {todayCount}
          </div>
        </div>
        <div className="text-center">
        ...
        <div className="text-center">
          ...
          <div className={`text-xl font-bold tabular-nums ${pnlClass(todayPnl)}`}>
            {fmt(todayPnl, 2)}
          </div>
        </div>
      </div>
...
function RiskTab({ trades }: { trades: Trade[] }) {
  ...
  const maxPct = Math.max(
    risk.concentration.reduce((m, c) => (c.tradePct > m ? c.tradePct : m), 0),
    1
  )
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          {
            label: "Today's P&L",
            value: fmt(risk.dailyPnl) + ' USDT',
            cls: pnlClass(risk.dailyPnl),
          },
          {
            label: "Week's P&L",
            value: fmt(risk.weeklyPnl) + ' USDT',
            cls: pnlClass(risk.weeklyPnl),
          },
          {
            label: 'Consecutive Losses',
            value: String(risk.consecutiveLosses),
            cls: risk.consecutiveLosses >= 3 ? 'text-red-400' : 'text-white',
          },
          {
            label: "Today's Trades",
            value: `${risk.todayTradeCount} / avg ${risk.avgDailyTrades.toFixed(0)}`,
            cls: risk.todayTradeCount > risk.avgDailyTrades * 1.5 ? 'text-amber-400' : 'text-white',
          },
        ].map((s) => (
          <div
            key={s.label}
            className="bg-[#0f1117] border border-white/[0.06] rounded-xl px-4 py-3"
          >
      ...
      <div className="bg-[#0f1117] border border-white/[0.06] rounded-xl p-4">
        <div className="text-xs text-white/30 uppercase tracking-widest font-medium mb-4">
            ...
            <div key={c.symbol} className="space-y-1">
              <div className="flex items-center justify-between text-xs">
                ...
                <div className="flex items-center gap-4 text-white/40">
                  ...
                  <span className={`font-semibold tabular-nums ${pnlClass(c.pnl)}`}>
                    {fmt(c.pnl, 2)} USDT
                  </span>
              ...
              <div className="h-1.5 bg-white/[0.05] rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    ...
                    background:
                      c.tradePct >= 60 ? '#ef4444' : c.tradePct >= 40 ? '#f59e0b' : '#7c3aed',
                  }}
                />
              </div>
            </div>
        ...
        {risk.concentration[0]?.tradePct >= 60 && (
          <p className="text-xs text-red-400/70 mt-3">
            ⚠ High concentration in {risk.concentration[0].symbol} — single-coin risk elevated
          </p>
        )}
      </div>
...
function SessionTab({ trades }: { trades: Trade[] }) {
  ...
  const maxAbs = Math.max(
    active.reduce((m, h) => (Math.abs(h.pnl) > m ? Math.abs(h.pnl) : m), 0),
    1
  )
  ...
  return (
    <div className="space-y-4">
      <div className="bg-[#0f1117] border border-white/[0.06] rounded-xl p-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            {
              label: 'Net P&L',
              value: fmt(session.todayPnl) + ' USDT',
              cls: pnlClass(session.todayPnl),
            },
        ...
        {session.bestHour && (
          <div className="bg-emerald-500/[0.07] border border-emerald-500/20 rounded-xl p-4">
            ...
            <div className="text-xs text-white/40 mt-1">
              Avg {fmt(session.bestHour.avgPnl, 3)} USDT · {session.bestHour.winRate.toFixed(0)}% WR
            </div>
          </div>
        )}
        {session.worstHour && (
          <div className="bg-red-500/[0.07] border border-red-500/20 rounded-xl p-4">
            ...
            <div className="text-xs text-white/40 mt-1">
              Avg {fmt(session.worstHour.avgPnl, 3)} USDT · {session.worstHour.winRate.toFixed(0)}%
              WR
            </div>
          </div>
        )}
      ...
      <div className="bg-[#0f1117] border border-white/[0.06] rounded-xl p-4">
        <div className="text-xs text-white/30 uppercase tracking-widest font-medium mb-3">
          Historical P&L by Hour (UTC)
        </div>
        ...
        <p className="text-[10px] text-white/20 mt-2">
          Hover for details · Ring = current UTC hour
        </p>
      </div>
    </div>
  )
              ...
              <div key={w.weekKey} className="space-y-1.5">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-3">
                    <span className={`text-xs font-semibold tabular-nums ${pnlClass(w.pnl)}`}>
                      {fmt(w.pnl, 2)} USDT
                    </span>
...
function GoalsTab({ trades }: { trades: Trade[] }) {
  ...
  const saveDaily = (v: number) => {
    setDailyGoal(v)
    localStorage.setItem('tradiary_daily_goal', String(v))
  }
  const saveWeekly = (v: number) => {
    setWeeklyGoal(v)
    localStorage.setItem('tradiary_weekly_goal', String(v))
  }
  ...
  const weeklyPnl = useMemo(
    () => trades.filter((t) => t.entry_time >= weekStart).reduce((s, t) => s + t.pnl, 0),
    [trades, weekStart]
  )
    ...
    return (
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          ...
          <span
            className={`text-sm font-bold tabular-nums ${exceeded ? 'text-emerald-400' : negative ? 'text-red-400' : 'text-white'}`}
          >
          ...
          <div className="flex items-center gap-4">
            <div className="text-center">
              <div className="text-2xl font-bold text-violet-400">
                {((goalHistory.hits / goalHistory.total) * 100).toFixed(0)}%
              </div>
              <div className="text-[10px] text-white/25 mt-0.5">Daily hit rate</div>
            </div>
            <div className="flex-1 space-y-1">
              <div className="text-xs text-white/40">
                {goalHistory.hits} out of {goalHistory.total} trading days hit the {dailyGoal} USDT
                daily goal
              </div>
              <div className="h-2 bg-white/[0.05] rounded-full overflow-hidden">
                <div
        ...
        <div className="overflow-y-auto max-h-[520px] divide-y divide-white/[0.03]">
          {recent.map((t) => {
            ...
            return (
              ...
              >
                <div
                  className={`w-1 h-8 rounded-full shrink-0 ${t.pnl >= 0 ? 'bg-emerald-500' : 'bg-red-500'}`}
                />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-semibold text-white">{t.symbol}</span>
                    <span
                      className={`text-[10px] px-1.5 py-0.5 rounded font-bold ${t.direction === 'long' ? 'text-emerald-400 bg-emerald-500/10' : 'text-red-400 bg-red-500/10'}`}
                    >
                      {t.direction === 'long' ? 'BUY' : 'SELL'}
                    </span>
                    {q && (
                      <span
                        className={`text-[10px] px-1.5 py-0.5 rounded font-bold ${gradeBg(q.grade)} ${gradeColor(q.grade)}`}
                      >
                        {q.grade} · {q.score}
                      </span>
                    )}
                  ...
                  <div className="text-[10px] text-white/25 mt-0.5">
                    {format(new Date(t.entry_time), 'MMM d, HH:mm')} UTC · {t.quantity} @{' '}
                    {t.entry_price.toFixed(2)}
                  </div>
                </div>
                <span className={`text-sm font-bold tabular-nums shrink-0 ${pnlClass(t.pnl)}`}>
                  {fmt(t.pnl, 4)}
                </span>
              </button>
            )
          ...
          {!selected && (
            <div className="h-full flex flex-col items-center justify-center text-center gap-2 py-16">
              ...
              <p className="text-sm text-white/20">
                Select any trade to get an AI explanation of what happened and how to improve.
              </p>
            </div>
          )}
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-3 gap-2">
                {[
                  ...
                  {
                    label: 'P&L',
                    value: fmt(selected.pnl, 4) + ' USDT',
                    cls: pnlClass(selected.pnl),
                  },
                  ...
                  <div key={s.label} className="bg-[#080a0f] rounded-lg px-3 py-2">
                    ...
                    <div
                      className={`text-xs font-semibold ${(s as { cls?: string }).cls ?? 'text-white'}`}
                    >
                      {s.value}
                    </div>
                  </div>
                      ...
                      {analysis
                        ...
                        .map((line, i) => {
                          ...
                          const cleaned = line
                            ...
                            .trim()
                          return isBold ? (
                            <p key={i} className="text-white font-semibold text-xs mt-3">
                              {cleaned}
                            </p>
                          ) : (
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <div className="text-xs text-white/30">
            Real-time behavioral intelligence · {trades.length} fills analyzed
          </div>
        </div>
        {activeWarnings.length > 0 && (
          <div className="ml-auto flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-red-500/10 border border-red-500/20">
            ...
            <span className="text-xs text-red-400 font-semibold">
              {activeWarnings.length} active alert{activeWarnings.length > 1 ? 's' : ''}
            </span>
          </div>
        )}
      </div>
      ...
      {activeWarnings.length > 0 && (
        <div className="space-y-2">
          {activeWarnings.map((w) => (
            <WarningCard key={w.id} w={w} onDismiss={() => dismiss(w.id)} />
          ))}
        </div>
      )}
      ...
      {activeWarnings.length === 0 && hasTrades && (
        <div className="flex items-center gap-2 px-4 py-3 bg-emerald-500/[0.07] border border-emerald-500/20 rounded-xl">
          ...
          <span className="text-xs text-emerald-300/80">
            No behavioral warnings right now — your trading patterns look disciplined.
          </span>
        </div>
      )}
      ...
      {!hasTrades && (
        <div className="text-center py-12 text-white/20 text-sm">
          Sync fills from Delta Exchange to activate the copilot.
        </div>
      )}
