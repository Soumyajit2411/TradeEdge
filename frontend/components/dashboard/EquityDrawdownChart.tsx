...
export function EquityDrawdownChart({ data }: Props) {
  ...
  if (!mounted) return (
    <div className="bg-[#0f1117] border border-white/[0.07] rounded-xl p-4">
      <div className="text-xs text-white/40 uppercase tracking-widest font-medium mb-4">Equity curve & drawdown</div>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 4, right: 4, left: 4, bottom: 0 }}>
            ...
            <YAxis
              ...
              tick={tickStyle} axisLine={false} tickLine={false} width={56}
            />
            ...
            <Tooltip
              ...
              formatter={(v) => { const n = Number(v); return [`${n >= 0 ? '+' : ''}${n.toFixed(4)} USDT`, 'Equity'] }}
            />
            <Area type="monotone" dataKey="equity" stroke={equityColor} strokeWidth={2} fill="url(#eqGrad)" dot={false} />
          </AreaChart>
        </ResponsiveContainer>
      </div>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} margin={{ top: 0, right: 4, left: 4, bottom: 0 }}>
            ...
            <YAxis
              ...
              tick={tickStyle} axisLine={false} tickLine={false} width={40}
            />
            <ReferenceLine y={0} stroke="rgba(255,255,255,0.08)" />
            <Tooltip
              contentStyle={{ background: '#1a1d27', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, fontSize: 12 }}
