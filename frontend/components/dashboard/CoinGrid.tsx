...
function pnlColor(v: number) { return v > 0 ? '#34d399' : v < 0 ? '#f87171' : '#6b7280' }
      ...
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={daily} margin={{ top: 1, right: 0, left: 0, bottom: 0 }}>
          <Bar dataKey="pnl" radius={[1, 1, 0, 0]}>
            {daily.map(d => <Cell key={d.date} fill={pnlColor(d.pnl)} fillOpacity={0.7} />)}
          </Bar>
        </BarChart>
      </ResponsiveContainer>
    </div>
...
function OhlcvChart({ candles, resolution }: { candles: Candle[]; resolution: Resolution }) {
  ...
  if (candles.length < 2) return (
    <div className="h-[260px] flex items-center justify-center text-white/20 text-xs">Not enough candle data</div>
  )

  const H = 260
  const ML = 72, MR = 10, MT = 8, MB = 24
  const VOL_H  = 44
  ...
  const yTicks = Array.from({ length: numTicks }, (_, i) =>
    minP + (i / (numTicks - 1)) * priceRange)

  const xTickEvery = Math.max(Math.floor(n / 6), 1)
  const xTicks     = Array.from({ length: n }, (_, i) => i).filter(i => i % xTickEvery === 0)

  const fmtX = (i: number) => resolution === '1d'
    ? new Date(candles[i].time * 1000).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })
    : new Date(candles[i].time * 1000).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: false })
      ...
      {hovered && hoverIdx !== null && (
        <div className="absolute top-2 left-[76px] bg-[#0c0e14] border border-white/[0.1] rounded-lg px-3 py-2 text-[11px] z-10 pointer-events-none">
          ...
          <div className="grid grid-cols-2 gap-x-4 gap-y-0.5">
            <span className="text-white/35">O</span><span className="text-white tabular-nums">{priceFmt(hovered.open)}</span>
            <span className="text-white/35">H</span><span className="text-emerald-400 tabular-nums">{priceFmt(hovered.high)}</span>
            <span className="text-white/35">L</span><span className="text-red-400 tabular-nums">{priceFmt(hovered.low)}</span>
            <span className="text-white/35">C</span>
            <span className={`tabular-nums font-semibold ${hovered.close >= hovered.open ? 'text-emerald-400' : 'text-red-400'}`}>
              {priceFmt(hovered.close)}
            </span>
            <span className="text-white/35">Vol</span><span className="text-white/50 tabular-nums">{hovered.volume.toLocaleString()}</span>
          </div>
        </div>
      )}

      {width > 0 && <svg width={width} height={H} style={{ display: 'block' }}>
        {/* Y grid lines */}
        {yTicks.map((p, i) => (
          <line key={i} x1={ML} y1={py(p)} x2={ML + plotW} y2={py(p)}
            stroke="rgba(255,255,255,0.04)" strokeWidth={1} />
        ))}

        {/* Volume section divider */}
        <line x1={ML} y1={H - MB - VOL_H - 3} x2={ML + plotW} y2={H - MB - VOL_H - 3}
        ...
        {candles.map((c, i) => {
          ...
          return (
            <rect key={`v${i}`}
              x={cx(i) - candleW / 2} y={volBarY(c.volume)}
              width={candleW} height={volBarH(c.volume)}
              fill={up ? 'rgba(52,211,153,0.3)' : 'rgba(248,113,113,0.3)'} rx={1} />
          )
        })}
        ...
        {/* Hover crosshair */}
        {hoverIdx !== null && (
          <line x1={cx(hoverIdx)} y1={MT} x2={cx(hoverIdx)} y2={H - MB}
            stroke="rgba(255,255,255,0.12)" strokeWidth={1} strokeDasharray="3 3" />
        )}

        {/* Y-axis labels */}
        {yTicks.map((p, i) => (
          <text key={i} x={ML - 5} y={py(p) + 3.5} textAnchor="end"
            fontSize={9} fill="rgba(255,255,255,0.3)">{priceFmt(p)}</text>
        ))}

        {/* X-axis labels */}
        {xTicks.map(i => (
          <text key={i} x={cx(i)} y={H - 6} textAnchor="middle"
            fontSize={9} fill="rgba(255,255,255,0.25)">{fmtX(i)}</text>
        ))}

        {/* Y-axis border */}
        <line x1={ML} y1={MT} x2={ML} y2={H - MB} stroke="rgba(255,255,255,0.07)" strokeWidth={1} />

        {/* Last close reference */}
        <line x1={ML} y1={py(candles[candles.length - 1].close)} x2={ML + plotW}
        ...
        {candles.map((_, i) => (
          <rect key={`h${i}`}
            x={cx(i) - plotW / n / 2} y={MT}
            width={plotW / n} height={H - MT - MB}
            fill="transparent"
...
function LiveMarketPanel({ ticker, symbol }: { ticker: LiveTicker; symbol: string }) {
  ...
  useEffect(() => {
    ...
    fetch(`${backendUrl(`/api/delta/candles/${symbol}`)}?resolution=${resolution}`)
      .then(r => r.json())
      .then(data => { if (alive) { setCandles(Array.isArray(data) ? data : []); setCandleLoad(false) } })
      .catch(() => { if (alive) { setCandleErr('Failed to load chart data'); setCandleLoad(false) } })
    return () => { alive = false }
  }, [symbol, resolution])
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-[#0f1117] border border-white/[0.06] rounded-xl p-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <span className="text-[10px] text-white/30 uppercase tracking-widest font-medium">Price Chart</span>
            ...
            {RESOLUTIONS.map(r => (
              <button key={r.key} onClick={() => setResolution(r.key)}
                className={`px-2 py-0.5 rounded text-[10px] font-medium transition-colors ${
                  resolution === r.key ? 'bg-violet-600 text-white' : 'bg-white/[0.04] text-white/30 hover:text-white/60'
                }`}>
                {r.label}
              </button>
            ))}
        ...
        {candleLoad ? (
          <div className="h-52 flex items-center justify-center text-white/20 text-xs">Loading candles…</div>
        ) : candleErr ? (
          <div className="h-52 flex items-center justify-center text-red-400/60 text-xs">{candleErr}</div>
        ) : (
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
        {([
          { label: 'Mark Price',       value: ticker.mark_price > 0 ? priceFmt(ticker.mark_price)  : '—', cls: 'text-white' },
          { label: 'Spot Price',       value: ticker.spot_price > 0 ? priceFmt(ticker.spot_price)  : '—', cls: 'text-slate-300' },
          { label: '24h Change',       value: `${fmt(ticker.change_24h, 2)}%`,                             cls: ticker.change_24h >= 0 ? 'text-emerald-400' : 'text-red-400' },
          { label: '24h Open',         value: ticker.open  > 0 ? priceFmt(ticker.open)  : '—',            cls: 'text-white/70' },
          { label: '24h Close',        value: ticker.close > 0 ? priceFmt(ticker.close) : '—',            cls: 'text-white/70' },
          { label: 'Mark/Spot Spread', value: spread > 0 ? `${spreadPct.toFixed(3)}%` : '—',               cls: spreadPct > 0.1 ? 'text-amber-400' : 'text-white/50' },
          { label: 'Volume 24h',       value: ticker.volume_24h   > 0 ? ticker.volume_24h.toLocaleString()                    : '—', cls: 'text-white/70' },
          { label: 'Turnover 24h',     value: ticker.turnover_24h > 0 ? `${(ticker.turnover_24h / 1_000_000).toFixed(2)}M` : '—', cls: 'text-white/70' },
        ] as { label: string; value: string; cls: string }[]).map(s => (
          <div key={s.label} className="bg-[#0f1117] border border-white/[0.06] rounded-xl px-3 py-3">
            <div className="text-[10px] text-white/25 uppercase tracking-wide mb-1.5">{s.label}</div>
      ...
      {(ticker.contract_type || ticker.underlying_asset_symbol || ticker.quote_asset_symbol) && (
        <div className="bg-[#0f1117] border border-white/[0.06] rounded-xl px-4 py-3 flex flex-wrap gap-6">
          {ticker.contract_type && (
            <div>
              <div className="text-[10px] text-white/25 uppercase tracking-wide mb-1">Contract Type</div>
              <div className="text-xs text-white font-semibold uppercase">{ticker.contract_type}</div>
            </div>
          )}
          {ticker.underlying_asset_symbol && (
            <div>
              <div className="text-[10px] text-white/25 uppercase tracking-wide mb-1">Underlying</div>
              <div className="text-xs text-white/70 font-medium">{ticker.underlying_asset_symbol}</div>
            </div>
          )}
          {ticker.quote_asset_symbol && (
            <div>
              <div className="text-[10px] text-white/25 uppercase tracking-wide mb-1">Quote Asset</div>
...
function EquityCurve({ trades }: { trades: Trade[] }) {
  ...
  useEffect(() => { setMounted(true) }, [])
  const data = useMemo(() => {
    const sorted = trades.filter(t => t.pnl !== 0).sort((a, b) => a.entry_time.localeCompare(b.entry_time))
    let cum = 0
    return sorted.map(t => {
  ...
  if (data.length < 2 || !mounted) return <div className="h-40 flex items-center justify-center text-white/20 text-xs">Not enough data</div>
    ...
    <ResponsiveContainer width="100%" height={160}>
      <LineChart data={data} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
        ...
        <YAxis tick={{ fontSize: 10, fill: '#ffffff30' }} tickLine={false} axisLine={false}
          tickFormatter={v => v.toFixed(2)} width={52} domain={[min * 1.05, max * 1.05]} />
        <ReferenceLine y={0} stroke="#ffffff15" strokeDasharray="3 3" />
        <Tooltip
          contentStyle={{ background: '#0f1117', border: '1px solid #ffffff15', borderRadius: 8, fontSize: 11 }}
          labelStyle={{ color: '#ffffff60' }}
          formatter={(v) => [fmt(Number(v), 4) + ' USDT', 'Equity']}
        />
        <Line type="monotone" dataKey="equity" stroke={color} strokeWidth={1.5} dot={false} fill="url(#eqGrad)" />
      </LineChart>
    </ResponsiveContainer>
  )
}
...
function DailyPnlChart({ trades }: { trades: Trade[] }) {
  ...
  useEffect(() => { setMounted(true) }, [])
  const daily = useMemo(() => calcDailyPnlMap(trades), [trades])
  if (!mounted || daily.length < 2) return <div className="h-32 flex items-center justify-center text-white/20 text-xs">Not enough data</div>
  return (
    <ResponsiveContainer width="100%" height={128}>
      <BarChart data={daily} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
        <XAxis dataKey="date" tick={{ fontSize: 9, fill: '#ffffff25' }} tickLine={false} axisLine={false}
          tickFormatter={v => v.slice(5)} interval="preserveStartEnd" />
        <YAxis tick={{ fontSize: 9, fill: '#ffffff25' }} tickLine={false} axisLine={false} width={44}
        ...
        <Bar dataKey="pnl" radius={[2, 2, 0, 0]}>
          {daily.map(d => <Cell key={d.date} fill={pnlColor(d.pnl)} fillOpacity={0.8} />)}
        </Bar>
      </BarChart>
    </ResponsiveContainer>
  )
...
function HourlyHeatmap({ trades }: { trades: Trade[] }) {
  ...
  const maxAbs = Math.max(hours.reduce((m, h) => Math.abs(h.pnl) > m ? Math.abs(h.pnl) : m, 0), 1)
  return (
    <div className="flex flex-wrap gap-1">
      {hours.map(h => {
        const intensity = Math.abs(h.pnl) / maxAbs
        const color = h.pnl >= 0 ? `rgba(52,211,153,${0.15 + intensity * 0.65})` : `rgba(248,113,113,${0.15 + intensity * 0.65})`
        return (
          <div key={h.hour} title={`${h.label}: ${fmt(h.pnl, 2)} USDT (${h.trades} fills)`}
...
function DirectionBreakdown({ trades }: { trades: Trade[] }) {
  ...
  return (
    <div className="grid grid-cols-2 gap-3">
      {[
        { label: 'LONG', count: longs.length, wins: longWins.length, pnl: longPnl, color: 'emerald' },
        { label: 'SHORT', count: shorts.length, wins: shortWins.length, pnl: shortPnl, color: 'red' },
      ].map(d => (
        <div key={d.label} className="bg-[#080a0f] border border-white/[0.06] rounded-xl p-3">
          <div className={`text-xs font-bold mb-2 ${d.color === 'emerald' ? 'text-emerald-400' : 'text-red-400'}`}>{d.label}</div>
          <div className="space-y-1 text-xs">
            <div className="flex justify-between"><span className="text-white/35">Trades</span><span className="text-white">{d.count}</span></div>
            <div className="flex justify-between"><span className="text-white/35">Win Rate</span>
              <span className="text-white">{d.count ? ((d.wins / d.count) * 100).toFixed(0) : 0}%</span></div>
            <div className="flex justify-between"><span className="text-white/35">Net PnL</span>
              <span className={pnlClass(d.pnl)}>{fmt(d.pnl, 2)}</span></div>
          </div>
        </div>
      ))}
    </div>
    ...
    <div className="space-y-3">
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <div className="flex gap-1 ml-auto">
          {sortBtns.map(b => (
            <button key={b.key} onClick={() => { setSortKey(b.key); setPage(0) }}
            ...
            {slice.map(t => (
              <tr key={t.id} className="border-b border-white/[0.03] hover:bg-white/[0.02] transition-colors">
                <td className="px-3 py-2">
                  <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${t.direction === 'long' ? 'text-emerald-400 bg-emerald-500/10' : 'text-red-400 bg-red-500/10'}`}>
                ...
                <td className={`px-3 py-2 font-semibold tabular-nums ${pnlClass(t.pnl)}`}>{fmt(t.pnl, 4)}</td>
                <td className={`px-3 py-2 tabular-nums ${pnlClass(t.pnl_percent)}`}>{fmt(t.pnl_percent, 2)}%</td>
                <td className="px-3 py-2 text-white/20 max-w-[180px] truncate">{t.notes}</td>
                <td className="px-3 py-2 text-white/25 whitespace-nowrap">{format(new Date(t.entry_time), 'MMM d, HH:mm')}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      {pages > 1 && (
        <div className="flex items-center justify-center gap-1">
          <button onClick={() => setPage(p => Math.max(0, p - 1))} disabled={page === 0}
            className="px-3 py-1 rounded-lg text-xs bg-white/[0.04] text-white/40 hover:text-white/70 disabled:opacity-30 transition-colors">←</button>
          <span className="text-xs text-white/25 px-2">{page + 1} / {pages}</span>
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-5">
        ...
        <div className="relative flex items-center justify-center w-24 h-24 shrink-0">
          ...
          <div className="text-center z-10">
            <div className="text-xl font-bold" style={{ color }}>{healthScore}</div>
            <div className="text-[10px] text-white/30 leading-none mt-0.5">health</div>
          </div>
        </div>
        <div>
          <div className="text-sm font-semibold text-white mb-0.5">{symbol} Psychological Health</div>
          <div className="text-xs text-white/40">
            {biases.reduce((s, b) => s + b.instances.length, 0)} bias signals detected across {biases.filter(b => b.instances.length > 0).length} pattern types
          </div>
        </div>
      </div>
      ...
      <div className="space-y-3">
        {biases.map(bias => {
          ...
          return (
            <div key={bias.type} className="bg-[#080a0f] border border-white/[0.06] rounded-xl overflow-hidden">
              <button onClick={() => setExpanded(isOpen ? null : bias.type)}
                className="w-full flex items-center gap-3 px-4 py-3 hover:bg-white/[0.02] transition-colors">
                <span className={`w-2 h-2 rounded-full shrink-0 ${c.bar}`} />
                <span className={`text-xs font-semibold flex-1 text-left ${c.text}`}>{bias.label}</span>
                {bias.instances.length > 0 && (
                  <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${c.bg} ${c.text}`}>{bias.instances.length}</span>
                )}
                <span className="text-[10px] text-white/25 ml-1">{SEVERITY_LABEL[bias.severity]}</span>
                <div className="w-16 h-1 bg-white/[0.05] rounded-full overflow-hidden ml-2 shrink-0">
                  <div className={`h-full ${c.bar} rounded-full`} style={{ width: `${bias.score}%` }} />
                </div>
              ...
              {isOpen && (
                <div className="px-4 pb-3 border-t border-white/[0.05]">
                  ...
                  {bias.instances.length === 0 ? (
                    <p className="text-xs text-white/20 italic">No instances detected — clean execution on this pattern.</p>
                  ) : (
                    <div className="space-y-1.5">
                      {bias.instances.map((inst, i) => (
                        <div key={i} className={`flex gap-2 text-xs px-2 py-1.5 rounded-lg ${c.bg}`}>
                          <span className={`shrink-0 font-bold ${c.text}`}>{'●'.repeat(inst.severity)}{'○'.repeat(3 - inst.severity)}</span>
                          <div>
                            <span className="text-white/60">{inst.reason}</span>
                            <span className="text-white/25 ml-2 text-[10px]">{inst.entryTime.slice(0, 16).replace('T', ' ')}</span>
                          </div>
                        </div>
                      ))}
                    </div>
...
function CoinDetailPage({
  symbol, stats, trades, ticker, biasReport, onBack,
}: {
  symbol: string
  stats?: SymbolStats
  trades: Trade[]
    ...
    <div className="space-y-4">
        ...
        <div>
          ...
          {ticker && (
            <div className="flex items-center gap-3 mt-1.5">
              <span className="text-xl font-semibold text-white tabular-nums">{ticker.mark_price.toFixed(4)}</span>
              <span className={`flex items-center gap-1 text-sm font-medium ${ticker.change_24h >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                {ticker.change_24h >= 0 ? <TrendingUp size={13} /> : <TrendingDown size={13} />}
                {fmt(ticker.change_24h, 2)}% 24h
              </span>
              <span className="text-xs text-white/25">Vol {ticker.volume_24h > 0 ? ticker.volume_24h.toLocaleString() : '—'}</span>
              <span className="text-xs text-white/25">Spot {ticker.spot_price > 0 ? ticker.spot_price.toFixed(4) : '—'}</span>
            </div>
          )}
        </div>
      </div>
      ...
      {stats && (
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-2">
          {[
            { label: 'Net PnL', value: fmt(stats.totalPnl, 2) + ' USDT', cls: pnlClass(stats.totalPnl) },
            ...
            <div key={s.label} className="bg-[#0f1117] border border-white/[0.06] rounded-xl px-3 py-2.5">
              <div className="text-[10px] text-white/30 uppercase tracking-wide mb-1">{s.label}</div>
              <div className={`text-sm font-bold tabular-nums ${s.cls || 'text-white'}`}>{s.value}</div>
            </div>
          ))}
        </div>
      )}
      ...
      <div className="pt-1">
        {tab === 'market' && (
          ticker ? (
            ...
            <div className="bg-[#0f1117] border border-white/[0.06] rounded-xl p-8 flex flex-col items-center justify-center gap-3 text-white/25">
              ...
              <p className="text-xs">Live data streams from Delta Exchange when the market is open.</p>
            </div>
          )
        )}
        ...
        {tab === 'overview' && (
          <div className="space-y-4">
            <div className="bg-[#0f1117] border border-white/[0.06] rounded-xl p-4">
              <div className="text-[10px] text-white/25 uppercase tracking-widest mb-3">Cumulative Equity</div>
              <EquityCurve trades={trades} />
            </div>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <div className="bg-[#0f1117] border border-white/[0.06] rounded-xl p-4">
                <div className="text-[10px] text-white/25 uppercase tracking-widest mb-3">Daily P&L</div>
                <DailyPnlChart trades={trades} />
              </div>
              <div className="bg-[#0f1117] border border-white/[0.06] rounded-xl p-4 space-y-4">
                <div>
                  <div className="text-[10px] text-white/25 uppercase tracking-widest mb-3">Long vs Short</div>
                  <DirectionBreakdown trades={trades} />
                </div>
              </div>
            </div>
            <div className="bg-[#0f1117] border border-white/[0.06] rounded-xl p-4">
              <div className="text-[10px] text-white/25 uppercase tracking-widest mb-3">Hourly P&L (UTC)</div>
...
export function CoinGrid({
  symbolStats, trades, liveTickers, liveLoading, liveError, biasReport,
}: {
  ...
  const rows = useMemo<CoinRow[]>(() => {
    ...
    if (liveTickers.length > 0)
      return liveTickers.map(t => ({ symbol: t.symbol, ticker: t, stats: statsBySymbol.get(t.symbol) }))
  ...
  const sorted = useMemo(() => {
    ...
    if (sort === 'change24h') return [...list].sort((a, b) => (b.ticker?.change_24h ?? -999) - (a.ticker?.change_24h ?? -999))
    if (sort === 'price')    return [...list].sort((a, b) => (b.ticker?.mark_price ?? 0) - (a.ticker?.mark_price ?? 0))
    if (sort === 'pnl')      return [...list].sort((a, b) => (b.stats?.totalPnl ?? -999) - (a.stats?.totalPnl ?? -999))
    return [...list].sort((a, b) => (b.ticker?.turnover_24h ?? -999) - (a.ticker?.turnover_24h ?? -999))
  }, [rows, sort, search])
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex gap-4 text-xs text-white/30">
        <span className="text-emerald-400/70">{symbolStats.filter(s => s.totalPnl > 0).length} profitable coins</span>
        <span className="text-red-400/70">{symbolStats.filter(s => s.totalPnl < 0).length} losing coins</span>
        <span className="text-white/20">{liveTickers.length || symbolStats.length} live markets</span>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
        {sorted.map(row => {
          ...
          const pf = row.stats ? (isFinite(row.stats.profitFactor) ? row.stats.profitFactor.toFixed(2) : '∞') : null
          const change24 = row.ticker?.change_24h ?? 0
          const price = row.ticker?.mark_price ?? 0

          return (
            <button key={row.symbol} onClick={() => setSelected(row.symbol)}
              ...
              <div className="flex items-center justify-between mb-2">
                ...
                <ArrowLeft size={11} className="text-white/15 group-hover:text-violet-400/50 rotate-180 transition-colors" />
              </div>
              <div className="text-lg font-bold tabular-nums text-white">{price > 0 ? price.toFixed(4) : '—'}</div>
              <div className={`text-xs mt-1 ${change24 >= 0 ? 'text-emerald-400/90' : 'text-red-400/90'}`}>
                24h {fmt(change24, 2)}%
              </div>
