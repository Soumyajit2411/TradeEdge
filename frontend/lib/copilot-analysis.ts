...
export function computeWarnings(trades: Trade[]): BehavioralWarning[] {
  ...
  if (streak >= 3) {
    ...
    const normalWins = closed
      .slice(streak)
      .filter((_, i) => i < 100 && closed.slice(streak)[i]?.pnl > 0).length
  ...
  const avgPerDay =
    Object.values(dailyMap).reduce((s, n) => s + n, 0) / Math.max(Object.keys(dailyMap).length, 1)
  ...
  if (closed.length >= 5) {
    ...
    if (lastLoss && lastTrade.id !== lastLoss.id && lastTrade.quantity > avgSize * 1.8) {
      warnings.push({
        ...
        action:
          'Reduce to your standard position size immediately. Do not try to recover losses in one trade.',
  ...
  const avgDailyLoss =
    dailyPnls.filter((p) => p < 0).reduce((s, p) => s + Math.abs(p), 0) /
    Math.max(dailyPnls.filter((p) => p < 0).length, 1)
  ...
  if (todayPnl < 0 && Math.abs(todayPnl) > avgDailyLoss * 1.5) {
    warnings.push({
      ...
      action:
        'Strong recommendation: stop trading for today. Come back tomorrow with a fresh start.',
    })
  }
...
export function computeRiskProfile(trades: Trade[]): RiskProfile {
  ...
  return {
    ...
    avgDailyLoss: lossDays.length
      ? Math.abs(lossDays.reduce((s, d) => s + d, 0) / lossDays.length)
      : 0,
  }
}
...
export function computeSessionProfile(trades: Trade[]): SessionProfile {
  ...
  const best = sorted[0]
    ? {
        ...
        avgPnl: sorted[0].pnl / sorted[0].trades,
        winRate: (sorted[0].wins / sorted[0].trades) * 100,
      }
    : null
  const worst = sorted[sorted.length - 1]
    ? {
        label: sorted[sorted.length - 1].label,
        avgPnl: sorted[sorted.length - 1].pnl / sorted[sorted.length - 1].trades,
        winRate: (sorted[sorted.length - 1].wins / sorted[sorted.length - 1].trades) * 100,
      }
    : null
...
export function computeWeeklyDiscipline(trades: Trade[]): WeekDiscipline[] {
  ...
  closed.forEach((t) => {
    ...
    const weekNum = Math.floor(
      (d.getTime() - new Date(d.getFullYear(), 0, 1).getTime()) / 604_800_000
    )
    ...
    .map(([key, wTrades]) => {
      ...
      for (const [, ts] of symGroups) {
        ...
        for (let i = 1; i < sorted.length; i++) {
          ...
          if (
            prevExit &&
            sorted[i - 1].pnl < 0 &&
            minutesBetween(prevExit, sorted[i].entry_time) < 30
          )
            revenge++
        }
      }
      ...
      for (const [, ts] of symGroups) {
        ...
        for (let i = 2; i < sorted.length; i++) {
          if (minutesBetween(sorted[i - 2].entry_time, sorted[i].entry_time) <= 45) {
            fomo++
            i += 2
          }
        }
      ...
      const avgDaily = allDayCounts.length
        ? allDayCounts.reduce((s, n) => s + n, 0) / allDayCounts.length
        : 10
      ...
      const worstDay = dailyVals.length
        ? Math.min(
            0,
            dailyVals.reduce((m, d) => (d < m ? d : m), 0)
          )
        : 0
...
export function scoreTradeQuality(trade: Trade, allTrades: Trade[]): TradeQualityScore {
  ...
  if (hs.trades >= 5) {
    ...
    factors.push({
      label: 'Time of Day',
      delta,
      detail: `${hs.label} UTC — ${(wr * 100).toFixed(0)}% historical win rate (${hs.trades} samples)`,
    })
  }
  ...
  let prevLosses = 0
  for (const t of prevSymTrades) {
    if (t.pnl < 0) prevLosses++
    else break
  }
  if (prevLosses >= 2) {
    ...
    score += delta
    factors.push({
      label: 'Entry After Losses',
      delta,
      detail: `${prevLosses} consecutive ${trade.symbol} losses preceded this entry — emotional entry risk`,
    })
  }
  ...
  if (allSizes.length >= 5) {
    ...
    if (ratio > 1.8) {
      ...
      factors.push({
        label: 'Position Size',
        delta,
        detail: `${ratio.toFixed(1)}× average size — oversized entry increases emotional pressure`,
      })
    } else if (ratio < 0.5) {
      const delta = -5
      score += delta
      factors.push({
        label: 'Position Size',
        delta,
        detail: `${ratio.toFixed(1)}× average size — undersized entry suggests hesitation`,
      })
    } else {
      const delta = 10
      score += delta
      factors.push({
        label: 'Position Size',
        delta,
        detail: `Normal position size (${ratio.toFixed(1)}× avg) — disciplined sizing`,
      })
    }
  }
  ...
  if (prevSymTrades.length > 0 && prevSymTrades[0].exit_time) {
    ...
    if (mins >= 0 && mins < 10 && prevSymTrades[0].pnl < 0) {
      ...
      factors.push({
        ...
        delta,
        detail: `Entered ${mins.toFixed(0)} min after a loss — insufficient time to reset emotionally`,
      })
    }
  }
  ...
  if (symTrades.length >= 5) {
    ...
    if (recentWinners.length >= 3 && trade.direction === dominantDir) {
      ...
      factors.push({
        ...
        delta,
        detail: `${trade.direction.toUpperCase()} aligns with your recent winning direction on ${trade.symbol}`,
      })
    }
  }
